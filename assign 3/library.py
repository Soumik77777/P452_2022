...
def montecarlo_imp(a, m, n, seed):
    prandom = pseudoRandoms(a, m, n, seed)
    val = 0
    for i in range(n):
        x = -np.log(1 - (np.exp(1) - 1) / np.exp(1) * prandom[i])
        val += (np.exp(1) - 1) / np.exp(1) * np.exp(-x ** 2) / np.exp(-x)
    return val / n
...
def shooting(f, a, b, y_a, y_b, h=0.01, tol=1e-6):
    z_a = 2
    X, Y = RK4_2(f, [a, y_a, z_a], b, h)
    if abs(Y[-1] - y_b) <= tol:
        return X, Y
    else:
        z_old = z_a
        y_old = Y[-1]
        z_a = float(input("Enter guess slope: "))
        X, Y = RK4_2(f, [a, y_a, z_a], b, h)
        if (y_old > y_b and Y[-1] < y_b) or (y_old < y_b and Y[-1] > y_b):
            z_a = z_a + (z_old - z_a) * (y_b - Y[-1]) / (y_old - Y[-1])
            X, Y = RK4_2(f, [a, y_a, z_a], b, h)
        return X, Y
